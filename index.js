...
let createFiles = () => {
  let data = JSON.stringify({"data1" : "data"});
  fs.rmdirSync(path.join(path.resolve(),"source"),{recursive: true});
  fs.rmdirSync(path.join(path.resolve(),"template"),{recursive: true});

  fs.mkdirSync(path.join(path.resolve(),"source/_component"),{recursive: true});
  fs.mkdirSync(path.join(path.resolve(),"source/_data"),{recursive: true});
  fs.mkdirSync(path.join(path.resolve(),"source/css"),{recursive: true});

  fs.mkdirSync(path.join(path.resolve(),"template/layout"),{recursive: true});
...
let serve = (port, first) => {
  ...
  if (first) {
    ...
    app._router.stack = app._router.stack.filter((ar) => {
      return !ar.route
    });
  }
  ...
  app.use(express.static('source'), (req, res, next) => {
