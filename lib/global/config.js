...
var allPaths = {
  ...
  component: os.platform() == 'win32' ? path.join(path.resolve(), "src/_component/") : path.join(path.resolve(), "src/_component/").replace(/\\/g, "/"),
  ...
  dbpath: path.join(path.resolve(), "db.json").replace(/\\/g, "/"),
...
let allFunction = {
  ...
  processPageData: (data, stat, fileData) => {
    ...
    let URL = (config.removeHTMLExt) ? fullpath.replace('index.html', '').replace('.html', '') : fullpath.replace('index.html', '');
    return {
...
let processArguments = () => {
  ...
  config.production = args['--production'] || args._[0] == 'server' ? config.production : !config.production;
  return args;
}
...
let validateConfig = () => new Promise((res, rej) => {
  ...
  config.websiteImage = configData.websiteImage || config.websiteImage;
  ...
  config.purgeCssFiles = configData.purgeCssFiles || config.purgeCssFiles;
