...
// All paths
var allPaths = {
  config: path.join(path.resolve(), "src/config.json").replace(/\\/g, "/"),
...
// All functions
let allFunction = {
  getNchar: function getNchar(str, n, max = 10) {
    let absN = 0;
  ...
  fixSlash : function fixSlash(str){
    if (str.includes(':\\')){
      str = str.replace(/:\\/g, '://');
    }
    return str.replace(/\\/g, '/')
  },
  deepfinder: function deepfinder(fullpath){
  ...
  processPageData : function processPageData(data, stat, fileData) {
  ...
  processPostData: function processPageData(data, stat, fileData) {
  ...
  processTemplatandLaouteData: function processLayoutData(data) {
    return data.replace(/(include\()/g, '$1_path+');
  },
  processLayoutName : function processLayoutName(name) {
...
let feedGen = (site) => new Promise((res, rej) => {
  ...
  let feedData = {
    ...
    link: site.data.sitedata.websiteurl || "https://example.com",
    copyright: config.feed.copyright || "",
  }
  if (config.feed.siteimage) {
    feedData.image=site.data.sitedata.siteimage;
  }
  if (config.feed.favicon) {
    feedData.favicon=site.data.sitedata.favicon;
  }
...
let generate = async (preLoadedSite) => {
  ...
  site.posts.forEach(async (post) => {
    ...
    files.push(
      fs.writeFile(path.join(allPaths.public, post.actualpath),
