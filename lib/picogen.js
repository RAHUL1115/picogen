...
let readAllData = () => new Promise(async (res, rej) => {
  ...
  allDataFiles.forEach((file) => {
    if(path.extname(file.name) == '.json'){
      dataJson[file.name.replace(".json", "")] = JSON.parse(file.data)
    } 
    if(path.extname(file.name) == '.csv') {
      dataCsv.csv[file.name.replace(".csv", "")] = csv.fromString(file.data)
    }
  })

  
  Promise.all(Object.values(dataCsv.csv)).then(content => {
    i=0
    for (const keys in dataCsv.csv) {
      dataCsv.csv[keys] = content[i]
      i++;
    }
    db.set("data", dataJson)
    db.set("csv", dataCsv.csv)
    res();
  }).catch((err) => {
    console.log(err);
    rej()
