...
var allPaths = {
  ...
  create: path.join(path.resolve(), "src/_create").replace(/\\/g, "/"),
...
let watcher = () => {
  watch([allPaths.config, allPaths.data, allPaths.component, allPaths.layout, allPaths.page, allPaths.template, allPaths.post, allPaths.static], {
    recursive: true
  }, async (evt, name) => {
...
let create = (data) => {
  if (data.data) {
    ...
    let templateFile = path.join(allPaths.create, 'template.ejs');
