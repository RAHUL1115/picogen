...
let processArguments = ()=>{
  ...
  shouldGenerateSitemap = args['--noreload'];
  shouldGenerateFeed = args['--noreload'];
  if (args._.length == 0){
    args._[0] = 'server';
  }
  return args;
...
let processPages = (site, page) => {
  ...
  site.pages.forEach((page,index)=>{
    page.actualpath = site.data?.sitedata?.removehtmlext ? page.actualpath.replace('.html','') : page.actualpath;
  });
  page.actualpath = site.data?.sitedata?.removehtmlext ? page.actualpath.replace('.html','') : page.actualpath;
...
let updateServer = (preLoadedSite) => {
  ...
  site.pages.forEach((page) => {
    let serverPath = (site?.data?.sitedata?.removehtmlext) ? page.fullpath.replace('.html','') : page.fullpath;
...
let sitemapGen = (site) => new Promise((res, rej) => {
  ...
  site.pages.forEach((page)=>{
    if (!(page.data.noindex)) {
      let thispage = site.data?.sitedata?.removehtmlext ?
        (page.name == 'index.html' ?
          ((page.path == '/') ?
            page.path :
            page.path + '/') :
          page.fullpath.replace('.html', '')) :
        page.fullpath;

      let priority = (thispage == '/') ?
        '1.0' :
        (thispage.split('/').length == 2) ?
        '0.80' :
        (thispage.split('/')[thispage.split('/').length - 1] == '') ?
        '0.80' :
        '0.64';

      let url = site.data?.sitedata?.websiteurl ? site.data.sitedata.websiteurl : `http://localhost:${port}`;

      sitemap += '\n\t\<url>';
      sitemap += `\n\t\t<loc>${url}${thispage}</loc>`;
...
// TODO complete feed generator
// TODO complete the documentation for same
...
let generate = async () => {
  ...
  site.pages.forEach(async (page) => {
    fullpath = site.data?.sitedata?.removehtmlextgen ?
      ((page.name == 'index.html') ?
        page.fullpath :
        page.fullpath.replace('.html', '')) :
      page.fullpath;
    ...
    files.push(
      fs.writeFile(path.join(allPaths.public, fullpath),
